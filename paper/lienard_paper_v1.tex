...
\item[v1.1 (planned).] R1 executed: all assets on vendor daily data; frozen v1.0 re-run reported side-by-side. \emph{[to be completed]}
\item[v1.2 (planned).] R2 (PMCMC, $N_p \ge 2000$) and R4 (conformal intervals). \emph{[to be completed]}
\item[v2.0 (planned).] R3 exogenous circuit inputs (realized-cap MVRV, order-book $R$, flow-based $L$, ETF-flow $F$); R5 live paper-trading log opened. \emph{[to be completed]}
\end{description}
